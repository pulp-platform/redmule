...
N_PROC="${N_PROC:-4}"
REDMULE_PARAM_SETS="${REDMULE_PARAM_SETS:-8x8 8x32}"
...
EOF
    ...
    make hw-build target="$Target" \
        RedmuleHeight="$RedmuleHeight" \
        RedmuleWidth="$RedmuleWidth" \
        ProbStall="${ProbStall:-}" || status=$?
    if [ "$status" -ne 0 ]; then
        continue
    fi
    # be verbose in ci regression
    python3 "$ScriptDir/bwruntests.py" -s --yaml -t $BASE_TIMEOUT -p "$N_PROC" \
